...
class fMRIData:
    ...
    """
    def __init__(self, data: npt.NDArray, TR: int, voxel_names: Optional[npt.ArrayLike] = None):
        if len(data.shape) > 2:
        ...
        if type(voxel_names) != np.ndarray and not voxel_names:
            ...
            if len(voxel_names) != self.get_n_voxels():
                raise ValueError(f"Voxel names ({len(voxel_names)}) does not match number of voxels {self.get_n_voxels()}")
    ...
    def get_voxel_by_name(self, name) -> Tuple[int, npt.NDArray]:
